...
RegisterCommand("resetpcd", function(source, args, rawCommand)
  -- /resetpcd
  local src = source;
  if IsPlayerAceAllowed(src, "Dead-Priorities.ResetPCD") or IsPlayerAceAllowed(src, "Dead-Priorities.Commands") or src <= 0 then
    onHold = false;
    inProgress = false;
	  resetPCD = true;
    currentCooldownTime = 0;
    TriggerClientEvent('Dead-Priorities:Update', -1, inProgress, onHold, currentCooldownTime);
    TriggerClientEvent('Dead-Priorities:DrawText', -1, Config.Options.resetDisplay:gsub("{MINS}", currentCooldownTime))
...
RegisterCommand("inprogress", function(source, args, rawCommand)
  -- /inprogress 
  local src = source;
  if IsPlayerAceAllowed(src, "Dead-Priorities.InProgress") or IsPlayerAceAllowed(src, "Dead-Priorities.Commands") or src <= 0 then
    onHold = false;
    inProgress = true;
	  resetPCD = false;
    currentCooldownTime = 0;
    ...
    TriggerClientEvent('Dead-Priorities:DrawText', -1, Config.Options.InProgressDisplay:gsub("{MINS}", currentCooldownTime))
...
RegisterCommand("onhold", function(source, args, rawCommand)
  -- /onhold 
  local src = source;
  if IsPlayerAceAllowed(src, "Dead-Priorities.OnHold") or IsPlayerAceAllowed(src, "Dead-Priorities.Commands") or src <= 0 then
    inProgress = false;
    onHold = true;
    resetPCD = false;	
    currentCooldownTime = 0;
    ...
    TriggerClientEvent('Dead-Priorities:Update', -1, inProgress, onHold, currentCooldownTime);
    TriggerClientEvent('Dead-Priorities:DrawText', -1, Config.Options.OnHoldDisplay:gsub("{MINS}", currentCooldownTime))
...
RegisterCommand("cooldown", function(source, args, rawCommand)
  -- /cooldown <time>
  local src = source;
  if IsPlayerAceAllowed(src, "Dead-Priorities.Cooldown") or IsPlayerAceAllowed(src, "Dead-Priorities.Commands") or src <= 0 then 
    if #args > 0 then 
      -- Check if number 
      if tonumber(args[1]) ~= nil then 
        ...
        TriggerClientEvent('Dead-Priorities:DrawText', -1, display)
    ...
    if not onHold and not inProgress and not resetPCD then 
      if Config.Options.AlwaysDisplay then 
        ...
        if currentCooldownTime <= 0 then 
          local display = Config.Options.CooldownDisplay:gsub("{MINS}", currentCooldownTime);
          TriggerClientEvent('Dead-Priorities:DrawText', -1, "");
        else 
          local display = Config.Options.CooldownDisplay:gsub("{MINS}", currentCooldownTime);
          TriggerClientEvent('Dead-Priorities:DrawText', -1, display);
        end
